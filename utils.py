...
def compute_min_cut_loss(g, out):
    ...
    temp_index = [i for i in range(len(D))]
    sparse_coo_index = torch.IntTensor([temp_index, temp_index])
    D = torch.sparse_coo_tensor(sparse_coo_index, D)
    # print('start calculating min_cut ... ')
    min_cut = (
            torch.matmul(torch.sparse.mm(A.transpose(1, 0), S).transpose(1, 0), S).trace()
            / torch.matmul(torch.sparse.mm(D.transpose(1, 0), S).transpose(1, 0), S).trace()
    )
    return min_cut.item()
...
def feature_prop(feats, g, k):
    ...
    for _ in range(k):
        ...
        feats = g.ndata.pop("h")
        feats = feats * norm
    neighbor_message = feats - origin_feats

    return feats, neighbor_message
